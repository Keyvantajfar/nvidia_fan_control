...
3. In another terminal, set fake temperatures and tail the mock fan-speed log
   ...
   iteration. `tail-log` now emits `[temp]` entries every time the daemon polls
   the sensor as well as `[temp=… speed=…]` entries whenever the fan speed is
   changed, so you can confirm both the readback temperature and any speed
   adjustments. The daemon's smoothing logic intentionally skips redundant
   writes, so repeated `set-temp` commands that map to the same fan speed will
   show up as `[temp]` reads without additional `speed=` lines.
